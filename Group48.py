...
class Advisor():
    ...
    def init_my_belief(self):
        ...
        community =[]
        if self.round_state != None:
            self.cd.update_deck(self.round_state['community_card'])
        ...
        my_belief = {'card':unsuit_card,
                    'strength': np.zeros(len(unsuit_card)),
                    'probability': np.zeros(len(unsuit_card))
                    }

        remove = []
        for i in range(len(my_belief['card'])):
            prob = self.cd.get_prob(my_belief['card'][i].ranks)
            if prob[0] == 0 or prob[1] == 0:
                remove.append(i)
            else:
                my_belief['probability'][i] = prob[0] * prob[1]
                # print(prob[0], prob[1])
        # self.cd.print_deck()
        # sleep(3)
        for item in ['card', 'probability', 'strength']:
            my_belief[item] = np.delete(my_belief[item], remove)

        strength = np.array(list(map(lambda x: heuristic1(x.to_hole_card(),community),my_belief['card'])))
        my_strength = heuristic1(self.my_card,community)
        my_strength = my_strength/np.sum(strength)

        sort_indices = np.argsort(strength)
        
        my_belief['card']= my_belief['card'][sort_indices]
        my_belief['probability'] = my_belief['probability'][sort_indices]
        ...
        for i in range(len(unique_values)):
            ...
            my_belief['strength'][indices] = (i+1.0)/len(unique_values)
    ...
    def suggest_action(self):
        ...
        self.pay_off_table = {}
        # print(printCards(self.my_card),np.sum(self.my_belief['probability']),self.my_belief['probability'])
        # sleep(3)    
        a_game_state = None
